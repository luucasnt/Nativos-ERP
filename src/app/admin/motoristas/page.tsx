              ...
              <li key={driver.id} className="surface-panel p-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-xs text-forest/60">{driver.supplier?.name ?? "Operação Nativos"}</p>
                  </div>
                  <span className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${driver.status === "ativo" ? "bg-success-light text-success" : "bg-forest/[0.07] text-forest/60"}`}>
                    {driver.status}
                  </span>
                </div>
                <dl className="mt-3 grid grid-cols-2 gap-3 text-xs">
                  <div><dt className="text-forest/55">Operação</dt><dd className="mt-1 font-medium text-forest">{driver.owner_type === "proprio" ? "Própria" : "Terceirizada"}</dd></div>
                  <div><dt className="text-forest/55">Aprovação</dt><dd className="mt-1 font-medium capitalize text-forest">{driver.approval_status}</dd></div>
                </dl>
