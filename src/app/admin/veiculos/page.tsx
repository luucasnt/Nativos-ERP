...
export default async function VeiculosPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const vehicles = await prisma.vehicle.findMany({
    where: query ? { OR: [{ plate: { contains: query, mode: "insensitive" } }, { model: { contains: query, mode: "insensitive" } }] } : undefined,
            ...
            {vehicles.map((vehicle) => (
              <li key={vehicle.id} className="surface-panel p-4">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${vehicle.status === "ativo" ? "bg-success-light text-success" : "bg-forest/[0.07] text-forest/60"}`}>
                    {vehicle.status}
                  </span>
                </div>
                <dl className="mt-3 grid grid-cols-2 gap-3 text-xs">
                  <div><dt className="text-forest/55">Categoria</dt><dd className="mt-1 font-medium text-forest">{vehicle.category?.label ?? "—"}</dd></div>
                  <div><dt className="text-forest/55">Aprovação</dt><dd className="mt-1 font-medium capitalize text-forest">{vehicle.approval_status}</dd></div>
                </dl>
                <p className="mt-3 text-xs text-forest/62">Fornecedor: <strong className="font-medium text-forest">{vehicle.supplier?.name ?? "Nativos"}</strong></p>
                <p className="mt-2 text-xs text-forest/62">Controle: <strong className="font-medium text-forest">{vehicle.expense_policy ? `${Number(vehicle.expense_policy.expected_km_per_liter).toFixed(1)} km/l` : "parâmetros não definidos"}</strong></p>
