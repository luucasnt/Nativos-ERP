...
export default async function SolicitacoesPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  return <div className="mx-auto max-w-[1280px] space-y-5">
    ...
    <nav aria-label="Etapas das solicitações" className="flex gap-1 overflow-x-auto border-b border-forest/10">{TABS.map((item) => <Link key={item.key} href={href(params, { tab: item.key, page: undefined })} aria-current={tab.key === item.key ? "page" : undefined} className={`focus-ring relative flex shrink-0 items-center gap-2 px-3 pb-3 pt-1 text-sm font-medium ${tab.key === item.key ? "text-forest" : "text-forest/52"}`}>{item.label}<span className="rounded-full bg-forest/[0.06] px-2 py-0.5 text-xs">{tabCount(item)}</span>{tab.key === item.key && <span className="absolute inset-x-2 bottom-0 h-0.5 bg-gold" />}</Link>)}</nav>
    <form className="surface-panel grid gap-3 p-4 md:grid-cols-[minmax(240px,1fr)_200px_auto]" action="/admin/solicitacoes" method="get"><input type="hidden" name="tab" value={tab.key} /><label className="relative"><span className="sr-only">Buscar solicitação</span><Search size={16} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-forest/42" /><input name="q" defaultValue={query} placeholder="Protocolo, reserva ou solicitante" className={`${inputClass} pl-9`} /></label><select name="categoria" defaultValue={params.categoria ?? ""} className={inputClass}><option value="">Todas as áreas</option><option value="operacional">Operacional</option><option value="financeiro">Financeiro</option></select><button className={secondaryButtonClass}><Filter size={15} /> Filtrar</button></form>
    ...
    {requests.length === 0 ? <section className="surface-panel py-14 text-center"><Inbox className="mx-auto text-forest/25" size={34} /><h2 className="mt-3 text-base font-semibold text-forest">Nenhuma solicitação encontrada</h2><p className="mt-1 text-sm text-forest/55">Não há itens nesta fila com os filtros atuais.</p></section> : <div className="grid gap-4 xl:grid-cols-2">{requests.map((request) => {
      ...
      return <article key={request.id} className={`surface-panel overflow-hidden ${overdue ? "border-danger/25" : ""}`}>
        ...
        <div className="grid gap-4 p-4 sm:grid-cols-[1fr_220px]"><div className="space-y-4"><dl className="grid grid-cols-[96px_1fr] gap-x-3 gap-y-2 text-sm"><dt className="text-forest/48">Solicitante</dt><dd className="font-medium text-forest">{requester}</dd><dt className="text-forest/48">Reserva</dt><dd>{request.reservation ? <Link href={`/admin/reservas/${request.reservation.id}`} className="font-semibold text-forest underline-offset-4 hover:underline">{request.reservation.code}</Link> : "Não vinculada"}</dd><dt className="text-forest/48">Recebida</dt><dd className="text-forest/72">{request.created_at.toLocaleString("pt-BR", { timeZone: "America/Bahia", dateStyle: "short", timeStyle: "short" })}</dd></dl>
          {requestDetails.length > 0 && <div className="rounded-xl bg-forest/[0.045] p-3"><p className="mb-2 text-xs font-semibold uppercase tracking-[0.08em] text-forest/48">Detalhes do pedido</p>{requestDetails.map((detail) => <p key={detail.label} className="mb-2 last:mb-0"><span className="block text-xs capitalize text-forest/48">{detail.label}</span><span className="text-sm leading-relaxed text-ink">{detail.value}</span></p>)}</div>}
        ...
        </div><div><div className="mb-3 flex items-center gap-2 text-xs text-forest/48"><Clock3 size={14} /> Prazo: {request.category === "financeiro" ? "2 horas" : "30 minutos"}</div><ChangeRequestReviewActions id={request.id} currentStatus={request.status} /></div></div>
      </article>;
    })}</div>}
